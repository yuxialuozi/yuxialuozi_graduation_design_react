...
for i in {0..49}; do
  ...
  RESPONSE=$(curl -s -X POST "$BASE_URL/tenants" \
    ...
    -d "{\"name\":\"$NAME\",\"contactPerson\":\"$CONTACT\",\"phone\":\"$PHONE\",\"email\":\"$EMAIL\",\"status\":\"active\"}")
...
for i in {0..59}; do
  ...
  FLOOR=$((FLOOR_INDEX + 1))
  ROOM_NUM="${BUILDING:0:1}-${FLOOR}0$((ROOM_INDEX + 1))"
  ...
  RESPONSE=$(curl -s -X POST "$BASE_URL/rooms" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json; charset=utf-8" \
    -d "{\"roomNo\":\"$ROOM_NUM\",\"building\":\"$BUILDING\",\"floor\":$FLOOR,\"area\":$AREA,\"monthlyRent\":$RENT,\"status\":\"vacant\"}")
...
for i in {0..49}; do
  ...
  RESPONSE=$(curl -s -X POST "$BASE_URL/contracts" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json; charset=utf-8" \
    -d "{\"tenantId\":$TENANT_ID,\"contractNo\":\"$CONTRACT_NUM\",\"startDate\":\"$START_DATE\",\"endDate\":\"$END_DATE\",\"amount\":$AMOUNT,\"status\":\"$STATUS\"}")

  CONTRACT_ID=$(echo $RESPONSE | grep -o '"id":[0-9]*' | head -1 | cut -d':' -f2)
  if [ -n "$CONTRACT_ID" ]; then
    CONTRACT_IDS+=($CONTRACT_ID)
    echo "✅ 合同 $((i+1))/50: $CONTRACT_NUM"
  fi
done
...
for i in {0..199}; do
  ...
  DUE_DATE="2025-$(printf '%02d' $MONTH)-25"
  ...
  curl -s -X POST "$BASE_URL/fees" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json; charset=utf-8" \
    -d "{\"tenantId\":$TENANT_ID,\"roomNo\":\"$ROOM_NO\",\"feeType\":\"$FEE_TYPE\",\"amount\":$AMOUNT,\"period\":\"$PERIOD\",\"dueDate\":\"$DUE_DATE\",\"status\":\"$STATUS\"}" > /dev/null
...
for i in {0..99}; do
  ...
  curl -s -X POST "$BASE_URL/maintenance" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json; charset=utf-8" \
    -d "{\"tenantId\":$TENANT_ID,\"roomNo\":\"$ROOM_NO\",\"type\":\"$TYPE\",\"description\":\"$DESCRIPTION\",\"priority\":\"$PRIORITY\"}" > /dev/null
